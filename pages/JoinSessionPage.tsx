...
const JoinSessionPage: React.FC = () => {
    ...
    const { workshops, updateSessionInState, updateParticipantRecordInState } = useContext(AppContext) as AppContextType;
    ...
    useEffect(() => {
        if (view !== 'status_view' || !session || session.status !== 'scheduled') return;
            ...
            (payload) => {
                ...
                if (updatedSession.status === 'live') {
                    const updateAndNavigate = async () => {
                        ...
                        const { error } = await supabase.from('session_participant_records').update({ attendance: 'present' }).eq('id', identifiedParticipant.record.id);
                        if (error) console.error("Failed to update attendance on real-time join:", error);
                        else updateParticipantRecordInState({ ...identifiedParticipant.record, attendance: 'present' });
                        
                        sessionStorage.setItem('workshop_session_user', JSON.stringify({
                            id: identifiedParticipant.participant.id, name: identifiedParticipant.participant.name, email: identifiedParticipant.participant.email, role: 'participant' }));
    ...
    }, [view, session, sessionId, navigate, identifiedParticipant, updateSessionInState, updateParticipantRecordInState]);
    ...
    const handleIdentify = async (e: React.FormEvent) => {
        ...
        if (participant && record) {
            ...
            if (session.status === 'live') {
                const { error } = await supabase.from('session_participant_records').update({ attendance: 'present' }).eq('id', record.id);
                if(error) console.error(error);
                else updateParticipantRecordInState({ ...record, attendance: 'present' });
            ...
            setError('This email is not registered for this workshop. Please check and try again.');
        }
    };

    if (isLoading && workshops.length === 0) return <div className="text-center p-10">Loading session details...</div>;
            ...
            <div className="flex items-center justify-center min-h-[calc(100vh-80px)] bg-gray-50 px-4">
                <div className="w-full max-w-md space-y-8">
                    <div>
                        ...
                        <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">Join Session</h2>
    ...
    if (view === 'status_view' && identifiedParticipant) {
        const { record } = identifiedParticipant;
        if (session.status === 'scheduled') {
            return (
                 <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] bg-gray-50 px-4 text-center">
                     <LogoIcon className="mx-auto h-12 w-auto text-primary" /><h2 className="mt-6 text-3xl font-extrabold text-gray-900">{workshop.title}</h2><p className="mt-2 text-xl text-gray-800">{session.title}</p><p className="mt-8 text-2xl text-gray-600 animate-pulse">The host has not started the session yet.</p><p className="mt-2 text-gray-500">This page will automatically update when the session begins.</p>
                 </div>
            );
        }
        if (session.status === 'ended') {
            ...
            return (
                 <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] bg-gray-50 px-4 text-center">
                     <LogoIcon className="mx-auto h-12 w-auto text-primary" /><h2 className="mt-6 text-3xl font-extrabold text-gray-900">{workshop.title}</h2><p className="mt-2 text-xl text-gray-800">{session.title}</p><p className="mt-4 text-lg text-gray-600">This session has already ended.</p>
                     {attended && (
                         <div className="mt-8"><p className="text-gray-700 mb-4">Thank you for your participation!</p><a href="https://echogb.typeform.com/to/cY2oFNyx" target="_blank" rel="noopener noreferrer" className="px-6 py-3 text-base font-medium text-white bg-primary rounded-md hover:bg-primary-700">Provide Feedback</a></div>
                     )}
                 </div>
            );
        }
